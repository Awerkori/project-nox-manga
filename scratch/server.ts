...
export const GET = async ({ locals, params, url }) => {
  ...
  if (parts[0] === 'me') {
    ...
    const { data: statsRows } = await safeQuery(
      db.get(sql`SELECT member_public_stats(${member.id}) as res`)
    );
    ...
    let conditions = [eq(schema.works.published, 1)] as any[];
    ...
    const { data: worksData } = await safeQuery(
      db.select({
        ...
        age_rating: schema.works.ageRating,
        published: schema.works.published,
        featured: schema.works.featured,
        cover_id: schema.works.coverId,
        source_id: schema.works.sourceId,
        updated_at: schema.works.updatedAt,
        created_at: schema.works.createdAt,
        search_text: schema.works.searchText,
        metadata_provenance: schema.works.metadataProvenance,
        content_rating: schema.works.contentRating,
        views_total: schema.works.viewsTotal,
        latest_chapter_published_at: schema.works.latestChapterPublishedAt
      })
      .from(schema.works)
    ...
    const { data: allowedRows } = await safeQuery(db.get(sql`SELECT public_chapter(${parts[1]}) as res`));
...
export const POST = async ({ locals, params, request }) => {
  ...
  if (parts[0] === 'events' && parts[1] === 'read') {
    ...
    const { data: viewResultRows } = await safeQuery(
      db.get(sql`SELECT record_chapter_view(${chapterId}, ${userId || null}, NULL, ${origin}) as res`)
    );
    ...
    if (userId && body.completed) {
      const { data: claimDataRows } = await safeQuery(
        db.get(sql`SELECT claim_chapter_xp(${chapterId}) as res`)
      );
