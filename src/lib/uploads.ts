    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const unzip = new Unzip((entry) => {
        ...
        if (
          ...
          failed = true;
          reject(new Error('O ZIP deve conter somente imagens PNG, JPEG ou WebP.'));
          return;
        }
        pending++;
        let size = 0;
        ...
        entry.ondata = (problem, chunk, final) => {
          ...
          if (problem) {
            failed = true;
            reject(new Error('ZIP corrompido.'));
            return;
          }
          size += chunk.length;
          total += chunk.length;
          if (size > 19_000_000 || total > 400_000_000 || result.length + pending > 500) {
            failed = true;
            entry.terminate();
            reject(new Error('Limite de 500 páginas, 19 MB por página ou 400 MB descompactados excedido.'));
            return;
          }
          chunks.push(chunk);
          if (final) {
      ...
      (async () => {
        ...
        try {
          ...
          failed = true;
          reject(new Error('Não foi possível abrir o ZIP.'));
        }
      })();
    });
  }
