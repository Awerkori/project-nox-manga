export function threadComments<T extends {id: string; parentId: string | null; createdAt: string}>(
  comments: T[]
): T[] {
  const roots = new Set(comments.filter((c) => !c.parentId).map((c) => c.id));
  const children = new Map<string, T[]>();
  for (const comment of comments)
    if (comment.parentId && roots.has(comment.parentId)) {
      const siblings = children.get(comment.parentId) || [];
      siblings.push(comment);
      children.set(comment.parentId, siblings);
    }
  return comments
    .filter((c) => !c.parentId || !roots.has(c.parentId))
    .flatMap((c) => [
      c,
      ...(children.get(c.id) || []).sort((a, b) => a.createdAt.localeCompare(b.createdAt))
    ]);
}
