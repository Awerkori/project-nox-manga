...
async function run() {
    const works = await turso.execute('SELECT count(*) as c FROM works;');
    console.log(`Turso works: ${works.rows[0].c}`);

    const chapters = await turso.execute('SELECT count(*) as c FROM chapters;');
    console.log(`Turso chapters: ${chapters.rows[0].c}`);

    const pages = await turso.execute('SELECT count(*) as c FROM pages;');
    console.log(`Turso pages: ${pages.rows[0].c}`);
}
run();
