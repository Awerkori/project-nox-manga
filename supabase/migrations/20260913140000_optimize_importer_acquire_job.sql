-- Migration: 20260913140000_optimize_importer_acquire_job.sql
-- Optimizes importer_acquire_job candidate selection using priority index and candidate batching.
-- Eliminates O(N) correlated subqueries across 50,000 queue rows.

CREATE INDEX IF NOT EXISTS idx_importer_queue_prio_run 
ON public.importer_queue (priority DESC, next_run_at ASC) 
