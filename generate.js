...
const code = `import { error, fail } from '@sveltejs/kit';
...
export const load = async ({ locals, params }) => {
  ...
  const [
    ...
    safeQuerySingle(db.select({ count: count() }).from(schema.reading).innerJoin(schema.chapters, eq(schema.reading.chapterId, schema.chapters.id)).innerJoin(schema.works, eq(schema.chapters.workId, schema.works.id)).where(and(eq(schema.reading.userId, member.id), sql\\\`\\\${schema.reading.completedAt} IS NOT NULL\\\`, sql\\\`\\\${schema.chapters.publishedAt} IS NOT NULL\\\`, eq(schema.works.published, 1)))),
    safeQuerySingle(db.select({ count: sql<number>\\\`count(distinct \\\${schema.library.workId})\\\` }).from(schema.library).innerJoin(schema.works, eq(schema.library.workId, schema.works.id)).where(and(eq(schema.library.userId, member.id), eq(schema.works.published, 1)))),
    safeQuerySingle(db.select({ count: count() }).from(schema.library).innerJoin(schema.works, eq(schema.library.workId, schema.works.id)).where(and(eq(schema.library.userId, member.id), eq(schema.library.status, 'COMPLETED'), eq(schema.works.published, 1)))),
    safeQuerySingle(db.select({ count: count() }).from(schema.library).innerJoin(schema.works, eq(schema.library.workId, schema.works.id)).where(and(eq(schema.library.userId, member.id), eq(schema.library.favorite, 1), eq(schema.works.published, 1)))),
    ...
    canViewFavorites
      ? safeQuery(
          ...
          .where(and(eq(schema.library.userId, member.id), eq(schema.library.favorite, 1), eq(schema.works.published, 1)))
    ...
    canViewReadingHistory
      ? safeQuery(
          ...
          .where(and(eq(schema.reading.userId, member.id), eq(schema.works.published, 1)))
    ...
    safeQuerySingle(db.select({ role: schema.accessRoles.role }).from(schema.accessRoles).where(and(eq(schema.accessRoles.userId, member.id), eq(schema.accessRoles.suspended, 0)))),
...
export const actions = {
  toggleScanPrivacy: async ({ request, locals }) => {
    ...
    const { error: rpcErr } = await safeQuery(
      db.update(schema.members)
        .set({ privacyShowScans: showScans ? 1 : 0, privacyScanMode: mode } as any)
  ...
  moderateUserScans: async ({ request, locals }) => {
    ...
    if (!targetUserId) return fail(400, { message: 'Usuário não informado' });
    const hideBadges = formData.get('hide_badges') === 'true';
    const { error: rpcErr } = await safeQuery(
      db.update(schema.members)
        .set({ adminHideScanBadges: hideBadges ? 1 : 0 } as any)
        .where(eq(schema.members.id, targetUserId))
    );
    if (rpcErr) return fail(400, { message: rpcErr.message });
